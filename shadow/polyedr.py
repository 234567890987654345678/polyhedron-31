...
class Polyedr:
    ...
    def __init__(self, file):
        ...
        self.edges_uniq()
        for i in self.stan_edges:
            a = (i.beg.x, i.fin.x, (i.beg.x + i.fin.x) / 2)
            if R3.is_good(a[2]) and (R3.is_good(a[0]) ^ R3.is_good(a[1])):
                self.sum_of_edges += i.length()
    ...
    def draw(self, tk):
        ...
        print("Сумма длин рёбер:",
              self.sum_of_edges)

    def edges_uniq(self):
        edges = {}
        for e in self.stan_edges:
            if (e.beg, e.fin) not in edges and (e.fin, e.beg) not in edges:
                edges[(e.beg, e.fin)] = e
        self.stan_edges = list(edges.values())
